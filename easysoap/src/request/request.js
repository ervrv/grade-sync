    ...
    that.getRequestXml = (callParams, params, opts) => {
        ...
        return Promise.all([reqEnvParams, reqParamsString])
            .then((reqParams) => {
                ...
                let rawRequestXML =  requestTpl({
                    'envelope': envParams,
                    'head'    : reqHeadParams,
                    'body'    : reqParamsString
                });
                const reg1 = /<[0-9]+\s+xmlns="http:\/\/performance.sfedu.ru">/gi;
				const reg2 = /<\/[0-9]+>/gi;
				//let cleanRequestXml = rawRequestXML.replace('</0>', '').replace('<0 xmlns="http://performance.sfedu.ru">', '');
				let cleanRequestXml = rawRequestXML.replace(reg2, '').replace(reg1, '');
				return cleanRequestXml;
            });
    };
    ...
    that.soapCall = (callParams, params, opts) => {
        ...
        return that.getRequestXml(callParams, params, opts)
            .then((requestXml) => {
				...
				const reg1 = /<[0-9]+\s+xmlns="http:\/\/performance.sfedu.ru">/gi;
				const reg2 = /<\/[0-9]+>/gi;
				let cleanRequestXml = requestXml.replace(reg2, '').replace(reg1, '');
                var headers = {};
